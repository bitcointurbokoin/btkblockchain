...
#if defined(NDEBUG)
#error "BTK cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubtk) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight >= 0 && nHeight <= 2) {
        ...
        nSubsidy = 4 * COIN;
    } else if (nHeight <= 2102400 && nHeight >= 1051201) {
        nSubsidy = 3 * COIN;
    } else {
        nSubsidy = .001 * COIN;
    }
...
{
    RenameThread("btk-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateBTKSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BTK: We use certain sporks during IBD, so check to see if they are
