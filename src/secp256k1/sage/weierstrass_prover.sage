...
# operating on affine and Bitcoin Turbo Koinobian coordinates, including the point at infinity
...
class btkobianpoint:
  def __init__(self, x, y, z, infinity=0):
    ...
    self.Infinity = infinity
  def __str__(self):
    return "btkobianpoint(X=%s,Y=%s,Z=%s,inf=%s)" % (self.X, self.Y, self.Z, self.Infinity)


def point_at_infinity():
  return btkobianpoint(1, 1, 1, 1)


def negate(p):
  if p.__class__ == affinepoint:
    return affinepoint(p.x, -p.y)
  if p.__class__ == btkobianpoint:
    return btkobianpoint(p.X, -p.Y, p.Z)
  assert(False)
...
def law_btkobian_weierstrass_add(A, B, pa, pb, pA, pB, pC):
  """Check whether the passed set of coordinates is a valid Bitcoin Turbo Koinobian add, given assumptions"""
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_btkobian_point(pA) +
               good_btkobian_point(pB) +
...
def law_btkobian_weierstrass_double(A, B, pa, pb, pA, pB, pC):
  """Check whether the passed set of coordinates is a valid Bitcoin Turbo Koinobian doubling, given assumptions"""
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_btkobian_point(pA) +
               good_btkobian_point(pB) +
...
def law_btkobian_weierstrass_add_opposites(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_btkobian_point(pA) +
               good_btkobian_point(pB) +
...
def law_btkobian_weierstrass_add_infinite_a(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_btkobian_point(pA) +
               good_btkobian_point(pB) +
...
def law_btkobian_weierstrass_add_infinite_b(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_btkobian_point(pA) +
               good_btkobian_point(pB) +
...
def law_btkobian_weierstrass_add_infinite_ab(A, B, pa, pb, pA, pB, pC):
  assumeLaw = (good_affine_point(pa) +
               good_affine_point(pb) +
               good_btkobian_point(pA) +
               good_btkobian_point(pB) +
...
laws_btkobian_weierstrass = {
  'add': law_btkobian_weierstrass_add,
  'double': law_btkobian_weierstrass_double,
  'add_opposite': law_btkobian_weierstrass_add_opposites,
  'add_infinite_a': law_btkobian_weierstrass_add_infinite_a,
  'add_infinite_b': law_btkobian_weierstrass_add_infinite_b,
  'add_infinite_ab': law_btkobian_weierstrass_add_infinite_ab
}


def check_exhaustive_btkobian_weierstrass(name, A, B, branches, formula, p):
  """Verify an implementation of addition of Bitcoin Turbo Koinobian points on a Weierstrass curve, by executing and validating the result for every possible addition in a prime field"""
  F = Integers(p)
    ...
    for zb in xrange(1, p):
      for pa in points:
        for pb in points:
          for ia in xrange(2):
            for ib in xrange(2):
              pA = btkobianpoint(pa.x * F(za)^2, pa.y * F(za)^3, F(za), ia)
              pB = btkobianpoint(pb.x * F(zb)^2, pb.y * F(zb)^3, F(zb), ib)
              for branch in xrange(0, branches):
                ...
                if r:
                  match = False
                  for key in laws_btkobian_weierstrass:
                    assumeLaw, require = laws_btkobian_weierstrass[key](A, B, pa, pb, pA, pB, pC)
...
def check_symbolic_btkobian_weierstrass(name, A, B, branches, formula):
  """Verify an implementation of addition of Bitcoin Turbo Koinobian points on a Weierstrass curve symbolically"""
  ...
  pA = btkobianpoint(ax * Az^2, ay * Az^3, Az, Ai)
  pB = btkobianpoint(bx * Bz^2, by * Bz^3, Bz, Bi)
  ...
  for branch in xrange(branches):
    ...
    for key in laws_btkobian_weierstrass:
      res[key].append((check_symbolic_function(R, assumeFormula, assumeBranch, laws_btkobian_weierstrass[key], A, B, pa, pb, pA, pB, pC), branch))
