...
void random_group_element_btkobian_test(secp256k1_gej *gej, const secp256k1_ge *ge) {
    secp256k1_fe z2, z3;
    do {
...
void test_num_btkobi(void) {
    ...
    const int btkobi5[10] = { 0, 1, -1, -1, 1, 0, 1, -1, -1, 1 };
    ...
    for (i = 0; i < 10; ++i) {
        ...
        CHECK(secp256k1_num_btkobi(&n, &order) == btkobi5[i]);
    }
    ...
    if (secp256k1_num_btkobi(&n, &order) == -1) {
        secp256k1_num_add(&n, &n, &n);
    }

    /* test residue */
    CHECK(secp256k1_num_btkobi(&n, &order) == 1);
    /* test nonresidue */
    secp256k1_num_add(&n, &n, &n);
    CHECK(secp256k1_num_btkobi(&n, &order) == -1);
    ...
    CHECK(secp256k1_num_btkobi(&order, &order) == 0);

    /* check one less than the order */
    secp256k1_scalar_set_int(&small, 1);
    secp256k1_scalar_get_num(&n, &small);
    secp256k1_num_sub(&n, &order, &n);
    CHECK(secp256k1_num_btkobi(&n, &order) == 1);  /* sage confirms this is 1 */
}
...
/* This compares btkobian points including their Z, not just their geometric meaning. */
...
void test_ge(void) {
     ...
     * The second in each pair of identical points uses a random Z coordinate in the Bitcoin Turbo Koinobian form.
    ...
    for (i = 0; i < runs; i++) {
        ...
        random_group_element_btkobian_test(&gej[2 + 4 * i], &ge[2 + 4 * i]);
        secp256k1_gej_set_ge(&gej[3 + 4 * i], &ge[3 + 4 * i]);
        random_group_element_btkobian_test(&gej[4 + 4 * i], &ge[4 + 4 * i]);
        for (j = 0; j < 4; j++) {
