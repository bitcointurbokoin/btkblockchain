...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total btk balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in btk/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in btk/kb\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"btkaddress\"\n"
            "\nReturn information about the given btk address.\n"

            "\nArguments:\n"
            "1. \"btkaddress\"     (string, required) The btk address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"btkaddress\", (string) The btk address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BTK address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are btk addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) btk address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"btkaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"btkaddress\"  (string, required) The btk address to use for the signature.\n"
