...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (encStatus == model->Locked || encStatus == model->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(model->requestUnlock(AskPassphraseDialog::Context::Send_BTK, true));
        if (!ctx.isValid()) {
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel()) {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid BTK address"));
        } else // Valid address
        {
            CPubKey pubkey;
            CKeyID keyid;
