...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::BTK);
    ...
    if (!settings.contains("nAnonymizeBTKAmount"))
        settings.setValue("nAnonymizeBTKAmount", 1000);

    nAnonymizeBTKAmount = settings.value("nAnonymizeBTKAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBTKAmount"))
        SoftSetArg("-anonymizebtkamount", settings.value("nAnonymizeBTKAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in btk.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBTKAmount:
            return QVariant(nAnonymizeBTKAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeBTKAmount:
            nAnonymizeBTKAmount = value.toInt();
            settings.setValue("nAnonymizeBTKAmount", nAnonymizeBTKAmount);
            emit anonymizeBTKAmountChanged(nAnonymizeBTKAmount);
            break;
        case CoinControlFeatures:
